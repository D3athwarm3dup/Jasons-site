...
export default function LeaveAReview({ projectSlug }: LeaveAReviewProps) {
  ...
  return (
    <div className="bg-[#FAF5EE] rounded-xl border border-[#E8DDD0] overflow-hidden">
      ...
      <button
        type="button"
        onClick={() => step === "closed" && setStep("pin")}
        className={`w-full flex items-center justify-between px-6 py-4 text-left transition-colors ${
          step === "closed" ? "hover:bg-[#F0E8DF] cursor-pointer" : "cursor-default"
        }`}
      >
      ...
      {/* PIN entry step */}
      {step === "pin" && (
        <form onSubmit={handleVerifyPin} className="px-6 pb-6 space-y-4 border-t border-[#E8DDD0]">
          <div className="pt-4">
            <p className="text-sm text-[#2C2C2C] font-medium mb-1">Enter your project PIN</p>
            <p className="text-xs text-[#8C8277] mb-3">
              Jason provided you with a unique PIN for this project. Enter it below to unlock the review form.
            </p>
            <input
              type="text"
              inputMode="numeric"
              ...
              className="w-full border border-[#E8DDD0] rounded-lg px-4 py-2.5 text-sm font-mono tracking-widest focus:outline-none focus:ring-2 focus:ring-[#8B5E3C] focus:border-transparent bg-white"
            />
            {pinError && (
              <p className="text-xs text-red-600 mt-1.5">{pinError}</p>
            )}
          </div>
          <div className="flex gap-3">
            ...
            <button
              type="button"
              onClick={() => { setStep("closed"); setPin(""); setPinError(""); }}
